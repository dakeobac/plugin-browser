...
export function ActiveAgentsPanel({ initialAgents }: { initialAgents: AgentInstanceSummary[] }) {
  const { data: agents = initialAgents } = useAgents();
  ...
  const isWorker = (a: AgentInstanceSummary) => a.agentName.startsWith("worker-");

  return (
    <>
      {viewingWorkerId && (
        <WorkerOutputModal
          agentId={viewingWorkerId}
          ...
          <div className="space-y-2">
            {liveAgents.map((agent) => {
              ...
              return (
                <div
                  key={agent.id}
                  className={`flex items-start gap-3 rounded-md px-2 py-2 transition-colors hover:bg-secondary/40 ${worker ? "cursor-pointer" : ""}`}
                  onClick={worker ? () => setViewingWorkerId(agent.id) : undefined}
                >
                  <span
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {worker && (
                        <span className="shrink-0 rounded-full bg-purple-500/10 px-2 py-0.5 text-[10px] font-medium text-purple-400">
                          orchestrator
                        </span>
                      )}
                      {agent.pluginSlug && (
